...
bk_err_t bk_byte_rtc_start(byte_rtc_option_t *option)
{
    ...
    LOGI("auto_subscribe_audio: %d, auto_subscribe_video: %d audio_codec_type:%d \n",
         rtc->byte_rtc_option.room_options.auto_subscribe_audio ? 1 : 0, 
		 rtc->byte_rtc_option.room_options.auto_subscribe_video ? 1 : 0,
		 rtc->byte_rtc_option.audio_data_type);
    ...
    LOGI("byte_rtc_join_room, room:%s uid:%s \n", room->room_id, room->uid);
