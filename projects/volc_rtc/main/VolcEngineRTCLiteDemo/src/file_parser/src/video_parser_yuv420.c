...
int yuv420_open(media_parser_t *h, const char *path)
{
  int rval = -1;
  ...
  do {
    ...
    //   break;
    // }
    mapped = volc_malloc(sb.st_size);
    if (mapped == NULL) {
      break;
    }

    ctx_t *p_ctx = (ctx_t *)volc_malloc(sizeof(ctx_t));
    if (!p_ctx) {
      break;
    }
...
#define LENGTH_PER_FRAME (240*240*1.5)
static int yuv420_obtain_frame(media_parser_t *h, frame_t *p_frame)
{
  int rval = -1;
  ...
  if (p_ctx->data_offset_ + LENGTH_PER_FRAME > p_ctx->data_size_) {
    ...
    AGO_LOGE("parser: LENGTH_PER_FRAME > data_size_");
    return -2;
  } else {
    p_frame->ptr = p_ctx->data_buffer_ + p_ctx->data_offset_;
...
static int yuv420_reset(media_parser_t *h)
{
  int rval = -1;
  do {
    ctx_t *p_ctx = (ctx_t *)h->p_ctx;
...
static int yuv420_close(media_parser_t *h)
{
  int rval = -1;
  do {
    ctx_t *p_ctx = (ctx_t *)h->p_ctx;
