...
static int32_t a2dp_sink_demo_vote_enable(uint8_t enable, uint32_t flag)
{
    ...
    //comment here if you need resume avrcp
    if (enable && current_play_entry != AUDIO_SOURCE_ENTRY_A2DP && current_play_entry != AUDIO_SOURCE_ENTRY_END)
    {
    ...
    if (AUDIO_SOURCE_ENTRY_STATUS_IDLE != s_audio_source_arbiter_entry_status
            && AUDIO_SOURCE_ENTRY_STATUS_PLAY != s_audio_source_arbiter_entry_status
