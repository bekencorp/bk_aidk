...
static void cmd_headset_demo(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    else if(os_strcmp(argv[1], "connect_cancel") == 0 && argc >= 3)
    {
        uint8_t mac_final[6] = {0};
        uint32_t mac[6] = {0};
        ...
        ret = bk_bt_gap_create_conn_cancel(mac_final);
    ...
    {
        uint8_t mode = 1; //BT_MNG_MODE_IDLE = 5

        if(argc >= 3)
        {
            ret = sscanf(argv[2], "%hhu", &mode);
...
#endif
    else if(os_strcmp(argv[1], "path_test") == 0 && argc >= 3)
    {
        uint32_t path = 0;

        ret = sscanf(argv[2], "%u", &path);
